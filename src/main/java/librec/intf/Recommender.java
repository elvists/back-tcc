...
public abstract class Recommender implements Runnable {
	...
	// configer
	public static ConfigurationData cf;
	...
	// is save main.java.br.ufba.model
	...
	public Recommender(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
		...
		if (resetStatics) {
			...
			cacheSpec = "maximumSize=200,expireAfterAccess=2m";

			rankOptions = cf.getParamOptions(cf.getItemRanking());
			...
			LineConfiger evalOptions = cf.getParamOptions(cf.getEvaluationSetup());
			...
			knn = cf.getNumNeighbors()  != null ? cf.getNumNeighbors() : 20;
			similarityMeasure = cf.getSimilarity()  != null ? cf.getSimilarity() : "PCC";
			similarityShrinkage = cf.getNumShrinkage() != null? cf.getNumShrinkage():30;
		}
	...
	public void execute() throws Exception {
		...
		if (Debug.ON) {
			// learn a recommender main.java.br.ufba.model
			initModel();

			// show algorithm's configuration
			printAlgoConfig();

			// build the main.java.br.ufba.model
			buildModel();

			// post-processing after building a main.java.br.ufba.model, e.g., release intermediate memory to avoid memory leak
			postModel();
		} else {
			/**
			 * load a learned main.java.br.ufba.model: this code will not be executed unless "Debug.OFF" mainly for the purpose of
	...
	/**
	 * validate main.java.br.ufba.model with held-out validation data
	 */
	...
	/**
	 * initilize recommender main.java.br.ufba.model
	 */
	protected void initModel() throws Exception {
	}

	protected LineConfiger getModelParams(String algoName) {
		return cf.getAlgoName()!=null ? cf.getParamOptions(algoName) : null;
	}
	...
	protected double correlation(SparseVector iv, SparseVector jv, String method) {
		...
		if (!Double.isNaN(sim)) {
			int n = is.size();
			int shrinkage = cf.getNumShrinkage();
			if (shrinkage > 0)
	...
	/**
	 * Learning method: override this method to build a main.java.br.ufba.model, for a main.java.br.ufba.model-based method. Default implementation is
	...
	/**
	 * After learning main.java.br.ufba.model: release some intermediate data to avoid memory leak
	 */
	protected void postModel() throws Exception {
	}

	/**
	 * Serializing a learned main.java.br.ufba.model (i.e., variable data) to files.
	 */
	protected void saveModel() throws Exception {
	}

	/**
	 * Deserializing a learned main.java.br.ufba.model (i.e., variable data) from files.
	 */
	...
	protected Map<Measure, Double> evalRatings() throws Exception {
		...
		for (MatrixEntry me : testMatrix) {
			...
			// perplexity: for some graphic main.java.br.ufba.model
